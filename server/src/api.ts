...
import { IsEmail, Length, IsDefined, Allow, IsOptional } from 'class-validator'
...
api.post('/admin/update-configs', reqAuth, async (req, res) => {
    await AppDataSource.createQueryBuilder().update(Configs).set(req.body).where("id = :id", {id: 1}).execute()
    res.send({ ok: true })
})
...
api.post('/admin/administrators', reqAuth, async (req, res) => {
    if (!req.body.id) {
        ...
        await AppDataSource.createQueryBuilder().update(Administrator).set(req.body).where("id = :id", {id: req.body.id}).execute()
    }
...
api.get('/admin/administrators', async (req, res) => {
    let r = await AppDataSource.getRepository(Administrator).find({order: {id: 'DESC'}, where: convertQStoObject(req.query)})
    res.send(r)
})
...
api.get('/admin/cliente-comprando', reqAuth, async (req, res) => {
    ...
    let r = await AppDataSource.getRepository(ClienteComprando).find({order: {id: 'DESC'}})
    res.send(r)
})
